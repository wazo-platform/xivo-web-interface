...
<?php
	...
	$form->text(array('desc' => $this->bbf('fm_auth_backend'),
                      'name'		=> 'auth_backend',
                      'labelid'	=> 'auth-backend',
                      'size'		=> 15,
                      'help'		=> $this->bbf('hlp_auth_backend'),
                      'default'	=> $element['auth_backend']['default'],
                      'value'		=> $info['auth_backend'],
                      'error'		=> $this->bbf_args('error',
                                                       $this->get_var('error', 'auth_backend')) )),
	$form->text(array('desc' => $this->bbf('fm_auth_host'),
                      'name'		=> 'auth_host',
                      'labelid'	=> 'auth-host',
                      'size'		=> 15,
                      'default'	=> $element['auth_host']['default'],
                      'value'		=> $info['auth_host'],
                      'error'		=> $this->bbf_args('error',
                                                       $this->get_var('error', 'auth_host')) )),
	$form->text(array('desc' => $this->bbf('fm_auth_port'),
                      'name'		=> 'auth_port',
                      'labelid'	=> 'auth-port',
                      'size'		=> 15,
                      'default'	=> $element['auth_port']['default'],
                      'value'		=> $info['auth_port'],
                      'error'		=> $this->bbf_args('error',
                                                       $this->get_var('error', 'auth_port')) )),
	$form->select(array('desc' => $this->bbf('fm_auth_verify_certificate_select'),
                        'name'		=> 'auth_verify_certificate_select',
                        'labelid'	=> 'auth-verify-certificate-select',
                        'key'		=> false,
                        'bbf'		=> 'fm_auth_verify_certificate_select-opt',
                        'bbfopt'	=> array('argmode' => 'paramvalue'),
                        'default'	=> $element['auth_verify_certificate_select']['default'],
                        'selected'	=> $info['auth_verify_certificate_select']),
                  $element['auth_verify_certificate_select']['value']),
	$form->text(array('desc' => $this->bbf('fm_auth_custom_ca_path'),
                      'name'		=> 'auth_custom_ca_path',
                      'labelid'	=> 'auth-custom-ca-path',
                      'size'		=> 30,
                      'help'		=> $this->bbf('hlp_custom_ca_path'),
                      'default'	=> $element['auth_custom_ca_path']['default'],
                      'value'		=> $info['auth_custom_ca_path'],
                      'error'		=> $this->bbf_args('error',
                                                       $this->get_var('error', 'auth_custom_ca_path')) )),
